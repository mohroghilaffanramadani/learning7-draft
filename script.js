...
document.getElementById("btnDaftar").addEventListener("click", function () {
  alert(
    "Terima kasih! Pendaftaran Anda akan segera diproses. Hubungi admin untuk info kelas minggu depan.",
  );
});
...
document.querySelectorAll(".btn-daftar-program").forEach((button) => {
  button.addEventListener("click", function () {
    ...
    alert(
      `✅ Pendaftaran untuk "${programName}" telah diterima!\nAdmin akan menghubungi Anda.`,
    );
  });
});
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    const nomorWA = "6282229093171"; // Format: 62 tanpa 0 di depan
    ...
    // Optional: Reset form atau beri notifikasi
    // this.reset(); // Uncomment jika ingin reset form setelah klik
    // alert('Terima kasih! Anda akan diarahkan ke WhatsApp untuk mengirim pendaftaran.');
  });
